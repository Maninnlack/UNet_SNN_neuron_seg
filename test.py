...
def test_func(test_img_path, test_mask_path, model_path_ANN, model_path_INN, device, T=5):
    prediced_img = []
    test_dataset = ImageDataset(test_img_path, test_mask_path, dtype='test')
    ...
    for _, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            pred_ANN = model_ANN(img_g)
            for t in range(T):
                if t == 0:
                    pred_INN = model_INN(img_g)
                else:
                    pred_INN += model_INN(img_g)
            pred_INN /= T
    ...
    plt.title('mask')
    plt.subplot(1,4,3)
    plt.imshow((pred_show_img_ANN ).float().cpu().numpy(), cmap='gray')
    plt.axis('off')
    plt.title('ANN result')
    plt.subplot(1,4,4)
    plt.imshow((torch.sigmoid(pred_show_img_INN)).float().cpu().numpy(), cmap='gray')
    plt.axis('off')
    plt.title('INN result')
    plt.savefig('./test_neuron_T.jpg', dpi=800)
    plt.show()
...
if __name__ == '__main__':
    ...
    model_path_INN = './model_save/INN_UNet_neuron_T.pth'
    # test_func(test_img_path, test_mask_path, model_path_ANN, model_path_INN, device=device)
