...
if __name__ == '__main__':
    ...
    x = torch.arange(-2.5, 2.5, 0.001)
    plt.plot(x.data, heaviside(x), label='阶跃函数', linestyle='-.')
    surrogate_function = ATan(alpha=3, spiking=False)
    y = surrogate_function(x)
    plt.plot(x.data, y.data, label='替代函数')

    surrogate_function = ATan(alpha=3, spiking=True)
    x.requires_grad_(True)
    y = surrogate_function(x)
    z = y.sum()
    z.backward()
    plt.plot(x.data, x.grad, label='替代函数梯度')
    plt.xlim(-2, 2)
    plt.legend()
